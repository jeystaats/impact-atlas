 ...
 *
 * Real-time air quality monitoring visualization with:
 * - AQI heatmap layer
 * - Monitoring station markers
 * - Wind dispersion animation
...
/** Generate monitoring stations for a city */
function generateMonitoringStations(cityLat: number, cityLng: number): MonitoringStation[] {
  return [
    { id: "ms-eixample", lat: cityLat + 0.006, lng: cityLng - 0.008, name: "Eixample", aqi: 120, pm25: 45, no2: 68, o3: 35, so2: 12, status: "online", type: "traffic" },
    { id: "ms-zona-franca", lat: cityLat - 0.030, lng: cityLng - 0.048, name: "Zona Franca", aqi: 140, pm25: 58, no2: 52, o3: 28, so2: 38, status: "online", type: "industrial" },
    { id: "ms-port-vell", lat: cityLat - 0.012, lng: cityLng + 0.018, name: "Port Vell", aqi: 95, pm25: 32, no2: 48, o3: 42, so2: 22, status: "online", type: "urban" },
    { id: "ms-placa-catalunya", lat: cityLat + 0.002, lng: cityLng - 0.003, name: "Placa Catalunya", aqi: 85, pm25: 28, no2: 55, o3: 38, so2: 8, status: "online", type: "urban" },
    { id: "ms-collserola", lat: cityLat + 0.045, lng: cityLng - 0.025, name: "Collserola", aqi: 35, pm25: 12, no2: 15, o3: 52, so2: 3, status: "online", type: "background" },
    { id: "ms-sant-adria", lat: cityLat + 0.025, lng: cityLng + 0.065, name: "Sant Adria", aqi: 110, pm25: 42, no2: 45, o3: 32, so2: 28, status: "online", type: "industrial" },
    { id: "ms-gracia", lat: cityLat + 0.018, lng: cityLng - 0.012, name: "Gracia", aqi: 75, pm25: 25, no2: 42, o3: 45, so2: 6, status: "online", type: "residential" },
    { id: "ms-poblenou", lat: cityLat + 0.008, lng: cityLng + 0.038, name: "Poblenou", aqi: 88, pm25: 30, no2: 52, o3: 40, so2: 15, status: "online", type: "urban" },
    { id: "ms-sants", lat: cityLat - 0.008, lng: cityLng - 0.035, name: "Sants", aqi: 92, pm25: 35, no2: 58, o3: 36, so2: 10, status: "maintenance", type: "traffic" },
    { id: "ms-diagonal", lat: cityLat + 0.012, lng: cityLng + 0.015, name: "Diagonal Mar", aqi: 68, pm25: 22, no2: 38, o3: 48, so2: 5, status: "online", type: "residential" },
  ];
}
...
function StationMarker({ station, isHighPollution, pulseScale, onClick }: StationMarkerProps) {
  return (
    <Marker longitude={station.lng} latitude={station.lat} anchor="center">
      <motion.div
      ...
      >
        ...
        >
          <span className="text-[8px] font-bold text-white">{station.aqi}</span>
        </div>
        <div
          className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-white ${
            station.status === "online" ? "bg-green-500" : station.status === "maintenance" ? "bg-yellow-500" : "bg-red-500"
          }`}
        />
      </motion.div>
    </Marker>
...
function StationPopup({ station, onClose }: StationPopupProps) {
  return (
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-2">
          <div
            ...
            style={{ backgroundColor: `${getAQIColor(station.aqi)}20` }}
          >
            <span className="text-sm font-bold" style={{ color: getAQIColor(station.aqi) }}>
              {station.aqi}
            </span>
          </div>
          <div>
            <h4 className="font-semibold text-[var(--foreground)] text-sm">{station.name}</h4>
            <p className="text-xs font-medium" style={{ color: getAQIColor(station.aqi) }}>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="flex justify-between">
            ...
            <span className="font-medium text-[var(--foreground)]">{station.pm25} ug/m3</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--foreground-muted)]">NO2:</span>
            <span className="font-medium text-[var(--foreground)]">{station.no2} ppb</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--foreground-muted)]">O3:</span>
            <span className="font-medium text-[var(--foreground)]">{station.o3} ppb</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--foreground-muted)]">SO2:</span>
            <span className="font-medium text-[var(--foreground)]">{station.so2} ppb</span>
          </div>
        </div>

        <div className="mt-2 pt-2 border-t border-[var(--border)] flex items-center justify-between">
          <span className="text-xs text-[var(--foreground-muted)] capitalize">{station.type} station</span>
          <span className={`text-xs font-medium ${station.status === "online" ? "text-green-500" : station.status === "maintenance" ? "text-yellow-500" : "text-red-500"}`}>
            {station.status}
          </span>
        </div>
      </motion.div>
...
interface WindParticleMarkerProps {
  particle: WindParticle;
}

function WindParticleMarker({ particle }: WindParticleMarkerProps) {
  return (
    <Marker longitude={particle.position.lng} latitude={particle.position.lat} anchor="center">
      <motion.div
        initial={{ scale: 0, opacity: 0 }}
        animate={{ scale: 1, opacity: particle.opacity * (1 - particle.age / particle.maxAge) }}
        exit={{ scale: 0, opacity: 0 }}
        className="rounded-full bg-white/60"
        style={{ width: 3, height: 3, boxShadow: "0 0 4px rgba(255, 255, 255, 0.5)" }}
      />
    </Marker>
  );
}
...
export function AirQualityMap({
  ...
  // Initialize stations
  useEffect(() => {
    const baseStations = generateMonitoringStations(city.coordinates.lat, city.coordinates.lng);
  ...
  useEffect(() => {
    if (stations.length > 0 && mapRef.current) {
      const points = stations.map((s) => ({ lat: s.lat, lng: s.lng }));
  ...
  const spawnWindParticle = useCallback(() => {
    ...
    const spawnLat = city.coordinates.lat - Math.cos(windRad) * spawnOffset + (Math.random() - 0.5) * 0.04;
    const spawnLng = city.coordinates.lng - Math.sin(windRad) * spawnOffset + (Math.random() - 0.5) * 0.04;

    return {
      id: `wind-${Date.now()}-${Math.random()}`,
      position: { lat: spawnLat, lng: spawnLng },
      velocity: { lat: Math.cos(windRad) * windSpeed * 0.0002, lng: Math.sin(windRad) * windSpeed * 0.0002 },
      age: 0,
      ...
      if (showWind) {
        setWindParticles((prev) => {
          ...
          return newParticles
            .map((p) => ({
              ...p,
              position: { lat: p.position.lat + p.velocity.lat, lng: p.position.lng + p.velocity.lng },
              age: p.age + 1,
            }))
  ...
  const heatmapGeojson = useMemo(() => {
    ...
    adjustedStations.forEach((station) => {
      ...
      features.push({
        type: "Feature",
        properties: { intensity: normalizedIntensity, aqi: station.aqi },
        geometry: { type: "Point", coordinates: [station.lng, station.lat] },
      });

      // Surrounding points for smoother heatmap
      for (let i = 0; i < 8; i++) {
        const angle = (Math.PI * 2 * i) / 8;
        const distance = 0.004 + Math.random() * 0.003;
        features.push({
          type: "Feature",
          properties: { intensity: normalizedIntensity * (0.5 + Math.random() * 0.3), aqi: station.aqi * (0.6 + Math.random() * 0.3) },
          geometry: { type: "Point", coordinates: [station.lng + Math.cos(angle) * distance, station.lat + Math.sin(angle) * distance] },
        });
      }
    });

    return { type: "FeatureCollection" as const, features };
  }, [adjustedStations, activePollutant]);
  ...
  const heatmapLayerStyle = useMemo(() => ({
    ...
    paint: {
      ...
      "heatmap-color": [
        ...
        0, "rgba(0, 0, 0, 0)",
        0.15, AQI_COLORS.good,
        0.35, AQI_COLORS.moderate,
        0.55, AQI_COLORS.unhealthySensitive,
        0.75, AQI_COLORS.unhealthy,
        0.9, AQI_COLORS.veryUnhealthy,
  ...
  const mapHeight = height - (showControls ? 220 : 160);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--foreground-muted)]">Real-time AQI with pollutant tracking and wind dispersion</p>
          </div>
        </div>
            ...
            {(Object.keys(TIME_LABELS) as TimeOfDay[]).map((time) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                  timeOfDay === time ? "bg-cyan-500/20 text-cyan-400" : "bg-[var(--background)] text-[var(--foreground-muted)] hover:text-[var(--foreground)]"
                }`}
              >
                {TIME_LABELS[time]}
              </button>
      ...
      >
        ...
        {/* Wind Particles */}
        <AnimatePresence>
          {showWind && windParticles.map((particle) => (
            <WindParticleMarker key={particle.id} particle={particle} />
          ))}
        </AnimatePresence>
          ...
          {adjustedStations.map((station) => {
            ...
            return (
              <StationMarker
                key={station.id}
                station={station}
                isHighPollution={isHighPollution}
        ...
        {showWind && (
          <div className="absolute bottom-4 left-4 bg-[var(--background)]/90 backdrop-blur-sm rounded-lg p-2 border border-[var(--border)]">
            <div className="flex items-center gap-2">
              <motion.div animate={{ rotate: windDirection }} className="w-6 h-6 flex items-center justify-center">
                <Icon name="arrowRight" className="w-4 h-4 text-[var(--foreground)]" />
              </motion.div>
              <div className="text-xs">
                <p className="text-[var(--foreground-muted)]">Wind</p>
                <p className="font-medium text-[var(--foreground)]">{Math.round(windSpeed * 15)} km/h</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-1">
            {AQI_THRESHOLDS.slice(0, 5).map((threshold, index) => (
              <div key={threshold.label} className="flex items-center">
                <div className="w-6 h-2 first:rounded-l last:rounded-r" style={{ backgroundColor: threshold.color }} title={threshold.label} />
                {index < 4 && <span className="text-[10px] text-[var(--foreground-muted)] mx-0.5">{threshold.max}</span>}
              </div>
            ))}
          </div>
      ...
      <div className="p-4 border-t border-[var(--border)]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  ...
                  style={{ backgroundColor: `${getAQIColor(avgAQI)}20`, color: getAQIColor(avgAQI) }}
                >
                  {getAQILabel(avgAQI)}
                </span>
              </div>
            ...
            <div>
              <p className="text-2xl font-bold text-[var(--foreground)]">{onlineStations}</p>
            ...
            <div>
              <div className="flex items-center gap-1">
                <Icon
                  ...
                  className={`w-4 h-4 ${aqiTrend.direction === "up" ? "text-red-500" : "text-green-500"}`}
                />
                <p className={`text-2xl font-bold ${aqiTrend.direction === "up" ? "text-red-500" : "text-green-500"}`}>
                  {aqiTrend.value}
                </p>
              </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${
                showHeatmap ? "bg-cyan-500/20 text-cyan-400" : "bg-[var(--background)] text-[var(--foreground-muted)]"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${
                showWind ? "bg-cyan-500/20 text-cyan-400" : "bg-[var(--background)] text-[var(--foreground-muted)]"
              }`}
            >
